...
},type,chainable?margin:void 0,chainable,null)}})}),jQuery.fn.size=function(){return this.length},jQuery.fn.andSelf=jQuery.fn.addBack;var _jQuery=window.jQuery,_$=window.$;return jQuery.noConflict=function(deep){return window.$===jQuery&&(window.$=_$),deep&&window.jQuery===jQuery&&(window.jQuery=_jQuery),jQuery},void 0===noGlobal&&(window.jQuery=window.$=jQuery),jQuery}),jQuery.noConflict(!0)}),define("taoQtiItem/portableLib/OAT/util/xml",[],function(){return{addNamespace:function(xml,ns){return ns?xml.replace(/<(\/)?([^!\/:>]+)(\/)?>/g,"<$1"+ns+":$2$3>"):xml},removeNamespace:function(markup){return markup.replace(/<(\/)?(\w*):([^>]*)>/g,"<$1$3>")}}}),
define("taoQtiItem/portableLib/OAT/util/math",["taoQtiItem/portableLib/jquery_2_1_1","mathJax"],function($,MathJax){"use strict";return{render:function($container){MathJax&&$container.find("math").each(function(){var $wrap,$math=$(this);$math.wrap($("<span>",{class:"math-renderer"})),$wrap=$math.parent(".math-renderer"),MathJax.Hub.Queue(["Typeset",MathJax.Hub,$wrap[0]])})}}}),function(window,document,undefined){!function(factory){"use strict";"function"==typeof define&&define.amd?define("taoQtiItem/portableLib/jquery.qtip",["taoQtiItem/portableLib/jquery_2_1_1"],factory):jQuery&&!jQuery.fn.qtip&&factory(jQuery)}(function($){"use strict"
;function QTip(target,options,id,attr){this.id=id,this.target=target,this.tooltip=NULL,this.elements={target:target},this._id=NAMESPACE+"-"+id,this.timers={img:{}},this.options=options,this.plugins={},this.cache={event:{},target:$(),disabled:FALSE,attr:attr,onTooltip:FALSE,lastClass:""},this.rendered=this.destroyed=this.disabled=this.waiting=this.hiddenDuringWait=this.positioning=this.triggering=FALSE}function invalidOpt(a){return a===NULL||"object"!==$.type(a)}function invalidContent(c){return!($.isFunction(c)||c&&c.attr||c.length||"object"===$.type(c)&&(c.jquery||c.then))}function sanitizeOptions(opts){var content,text,ajax,once
;return invalidOpt(opts)?FALSE:(invalidOpt(opts.metadata)&&(opts.metadata={type:opts.metadata}),"content"in opts&&(content=opts.content,invalidOpt(content)||content.jquery||content.done?(text=invalidContent(content)?FALSE:content,content=opts.content={text:text}):text=content.text,"ajax"in content&&(ajax=content.ajax,once=ajax&&ajax.once!==FALSE,delete content.ajax,content.text=function(event,api){var loading=text||$(this).attr(api.options.content.attr)||"Loading...",deferred=$.ajax($.extend({},ajax,{context:api})).then(ajax.success,NULL,ajax.error).then(function(newContent){return newContent&&once&&api.set("content.text",newContent),newContent
},function(xhr,status,error){api.destroyed||0===xhr.status||api.set("content.text",status+": "+error)});return once?loading:(api.set("content.text",loading),deferred)}),"title"in content&&($.isPlainObject(content.title)&&(content.button=content.title.button,content.title=content.title.text),invalidContent(content.title||FALSE)&&(content.title=FALSE))),"position"in opts&&invalidOpt(opts.position)&&(opts.position={my:opts.position,at:opts.position}),"show"in opts&&invalidOpt(opts.show)&&(opts.show=opts.show.jquery?{target:opts.show}:opts.show===TRUE?{ready:TRUE}:{event:opts.show}),"hide"in opts&&invalidOpt(opts.hide)&&(opts.hide=opts.hide.jquery?{
target:opts.hide}:{event:opts.hide}),"style"in opts&&invalidOpt(opts.style)&&(opts.style={classes:opts.style}),$.each(PLUGINS,function(){this.sanitize&&this.sanitize(opts)}),opts)}function convertNotation(options,notation){for(var obj,i=0,option=options,levels=notation.split(".");option=option[levels[i++]];)i<levels.length&&(obj=option);return[obj||options,levels.pop()]}function setCallback(notation,args){var category,rule,match;for(category in this.checks)if(this.checks.hasOwnProperty(category))for(rule in this.checks[category])this.checks[category].hasOwnProperty(rule)&&(match=new RegExp(rule,"i").exec(notation))&&(args.push(match),
("builtin"===category||this.plugins[category])&&this.checks[category][rule].apply(this.plugins[category]||this,args))}function createWidgetClass(cls){return WIDGET.concat("").join(cls?"-"+cls+" ":" ")}function delay(callback,duration){if(duration>0)return setTimeout($.proxy(callback,this),duration);callback.call(this)}function showMethod(event){this.tooltip.hasClass(CLASS_DISABLED)||(clearTimeout(this.timers.show),clearTimeout(this.timers.hide),this.timers.show=delay.call(this,function(){this.toggle(TRUE,event)},this.options.show.delay))}function hideMethod(event){if(!this.tooltip.hasClass(CLASS_DISABLED)&&!this.destroyed){
...
}(),$container.find(".svghome").html(svgstring),$container.find(".zonesocle").append($("<img>",{src:assetManager.resolve("delor/runtime/img/orus.png")}).attr("class","draggable catgod")),$container.find(".robi").append("<div class='modifeau'>Modifier le volume d'eau</div><div class='plus'>Plus +</div><input type='range' class='vanne' step='5' min='0' ma='140'><div class='moins'>- Moins</div>");var $vanne=$container.find(".vanne");$container.find(".g5982").click(function(event){1===statueloc?(tare=-190,$container.find(".tspan5974").html("0 g")):0!==statueloc&&2!==statueloc||(tare=0,$container.find(".tspan5974").html(tare+" g"))}),
$container.find(".rect5659-1").attr("height",0),$vanne.val(0);var oldVal;$vanne.on("input",function(){var newVal=$container.find(this).val();newVal>oldVal&&($container.find(".rect5659-1-1").css("height","455px"),$container.find(".rect5659-1-1").show()),oldVal=newVal}),$vanne.mouseup(function(event){"not-use"==answvanne&&($container.find(".deloransw").append(" vanne-ok,"),answvanne="vanne-ok"),obBecher.fillup(),2===statueloc&&0===niveau?(niveau=$vanne.val(),$container.find(".rect5659-1-1").hide(),niveau>97?(niveau=97,$container.find(".lecvol").html("Lecture impossible du volume"),$container.find(".deborde").show(),
$container.find(".rect5659-1").attr("height",4*niveau)):niveau>85&&niveau<97?($container.find(".lecvol").html("Lecture impossible du volume"),$container.find(".rect5659-1").attr("height",4*niveau),$container.find(".deborde").hide()):($container.find(".lecvol").html("Volume lu : "+niveau+" mL"),$container.find(".deborde").hide(),$container.find(".rect5659-1").attr("height",4*niveau))):(niveau=$vanne.val(),$container.find(".rect5659-1-1").hide(),niveau>85&&niveau<97?($container.find(".lecvol").html("Lecture impossible du volume"),$container.find(".rect5659-1").attr("height",4*niveau),$container.find(".deborde").hide()):niveau>97?(niveau=97,
$container.find(".lecvol").html("Lecture impossible du volume"),$container.find(".deborde").show(),$container.find(".rect5659-1").attr("height",4*niveau)):(niveau=$vanne.val(),$container.find(".lecvol").html("Volume lu : "+niveau+" mL"),$container.find(".deborde").hide(),$container.find(".rect5659-1").attr("height",4*niveau)))}),$container.find(".tspan5974").hide(),$container.find(".tspan5974").attr("x",480).attr("y",1e3),$container.find(".image5962").click(function(event){0===onoff?($container.find(".tspan5974").show(),$container.find(".path4217").css("fill","#00ff00"),$container.find(".tspan5974").css("opacity",1),
...
;var h=0,a=!0;if(u.push(n),o.push(t),p){if(s=n.length,h=t.length,(a=h==s)||r)for(;h--;)if(p=s,g=t[h],r)for(;p--&&!(a=at(n[p],g,e,r,u,o)););else if(!(a=at(n[h],g,e,r,u,o)))break}else nr(t,function(t,i,l){return we.call(l,i)?(h++,a=we.call(n,i)&&at(n[i],t,e,r,u,o)):void 0}),a&&!r&&nr(n,function(n,t,e){return we.call(e,t)?a=-1<--h:void 0});return u.pop(),o.pop(),l&&(c(u),c(o)),a}function it(n,t,e,r,u){(qe(t)?Dt:tr)(t,function(t,o){var a,i,l=t,f=n[o];if(t&&((i=qe(t))||er(t))){for(l=r.length;l--;)if(a=r[l]==t){f=u[l];break}if(!a){var c;e&&(l=e(f,t),c=void 0!==l)&&(f=l),c||(f=i?qe(f)?f:[]:er(f)?f:{}),r.push(t),u.push(f),c||it(f,t,e,r,u)}
}else e&&void 0===(l=e(f,t))&&(l=t),void 0!==l&&(f=l);n[o]=f})}function lt(n,t){return n+de(Fe()*(t-n+1))}function ft(e,r,u){var a=-1,l=ht(),f=e?e.length:0,s=[],g=!r&&f>=_&&l===n,h=u||g?i():s;for(g&&(h=o(h),l=t);++a<f;){var v=e[a],y=u?u(v,a,e):v;(r?!a||h[h.length-1]!==y:0>l(h,y))&&((u||g)&&h.push(y),s.push(v))}return g?(c(h.k),p(h)):u&&c(h),s}function ct(n){return function(t,e,r){var u={};if(e=v.createCallback(e,r,3),qe(t)){r=-1;for(var o=t.length;++r<o;){var a=t[r];n(u,a,e(a,r,t),t)}}else Xe(t,function(t,r,o){n(u,t,e(t,r,o),o)});return u}}function pt(n,t,e,r,u,o){var a=1&t,i=4&t,l=16&t,f=32&t;if(!(2&t||jt(n)))throw new le;l&&!e.length&&(t&=-17,l=e=!1),
...
define("delor/runtime/delor.amd",["qtiCustomInteractionContext","taoQtiItem/portableLib/jquery_2_1_1","delor/runtime/js/renderer","taoQtiItem/portableLib/OAT/util/event"],function(qtiCustomInteractionContext,$,renderer,event){"use strict";var delor={id:-1,getTypeIdentifier:function(){return"delor"},initialize:function(id,dom,config,assetManager){event.addEventMgr(this);this.id=id,this.dom=dom,this.config=config||{},renderer.render(this.id,this.dom,this.config,assetManager),qtiCustomInteractionContext.notifyReady(this)},setResponse:function(response){$(this.dom)},getResponse:function(){var $container=$(this.dom),value=$container.find(".deloransw").text()
;return value=value.slice(0,-1),{base:{string:value}}},resetResponse:function(){$(this.dom).find("input").prop("checked",!1)},destroy:function(){$(this.dom).off().empty()},setSerializedState:function(state){},getSerializedState:function(){return{}}};qtiCustomInteractionContext.register(delor)}),define(["delor/runtime/delor.amd"],function(PCI){return PCI});
//# sourceMappingURL=delor.amd..min.js.map